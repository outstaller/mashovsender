...
import pandas as pd
from mashov_api import MashovClient, StudentRow, read_students, compose, DEFAULT_SUBJECT, DEFAULT_TEMPLATE
...
def main():
    ...
    user, pwd, year, semel = os.getenv("MASHOV_USER"), os.getenv("MASHOV_PASS"), os.getenv("MASHOV_YEAR"), os.getenv("MASHOV_SEMEL")
    ...
    if recipients is None or len(recipients)==0: 
        print("No recipients found, cannot continue")
        exit(1)
    groups=[gr.get("displayName") for gr in recipients if gr.get("valueType")=="ClassNum" and gr.get("targetType")=="Students"]
    for f in groups:
        print(f)
    """
